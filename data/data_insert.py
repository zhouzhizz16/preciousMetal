...
def index_data_insert():
    ...
    eco_index_name_list = set()
    with open(data_input_file, 'rb') as f:
        reader = csv.reader(f)

        for row in reader:
            eco_index_name_list.add(row[5])


    for eco_ind_name in eco_index_name_list:
        if eco_ind_name == '指标名称':
            continue
        print 'processing eco_index: ',eco_ind_name
        with open(data_input_file, 'rb') as f:
            reader = csv.reader(f)
